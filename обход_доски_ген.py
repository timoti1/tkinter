...
def фитнес(генотип_агента):
    ...
    удачных_ходов = 0
    x_from, y_from = генотип_агента[0]
    for x, y in генотип_агента[1:]:
        ...
        #проверим "битый" ли ген
        if (x - x_from, y - y_from) not in теоретически_возможные_движения:
            return удачных_ходов
...
def эволюция(число_поколений):
    ...
    for i in range(число_поколений):
        лидеры = список_лучших(10)
        потомки = список_потомков(лидеры, 10)
