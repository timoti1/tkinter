...
def check_if_there_are_forgotten_cells(множество_достижимых_клеток):
    if len(множество_достижимых_клеток) == 64:
        return None

    все_множество_клеток = {(i, j) for i in range(1, 9) for j in range(1, 9)}
    клетки_для_проверки = все_множество_клеток - множество_достижимых_клеток
    теоретически_возможные_движения = get_figure_movements(gui.get_selected_figure())

    for x, y in клетки_для_проверки:
        есть_возможность_походить = False
        ...
        for смещение_x, смещение_y in теоретически_возможные_движения:
            ...
            if 1 <= потенциально_достижимая_клетка[0] <= 8 and 1 <= потенциально_достижимая_клетка[1] <= 8:
                if потенциально_достижимая_клетка not in множество_достижимых_клеток:
                    есть_возможность_походить = True
                    break

        if not есть_возможность_походить:
            break

    return not есть_возможность_походить
...
def board_traversal(волны, номер_волны, множество_достижимых_клеток):
    """алгоритм обхода конем всей доски, реализующий рекурсивный подход в процессе перебора"""

    def go_back(волны, номер_волны, множество_достижимых_клеток):
        if gui.get_debug():
            print('Откат назад. Волна {}'.format(номер_волны))
    ...
    #функция может работать долго. нужно периодически обрабатывать события
    gui.окно.update_idletasks()
    gui.окно.update()
            ...
            if 1 <= потенциально_достижимая_клетка[0] <= 8 and 1 <= потенциально_достижимая_клетка[1] <= 8:
                if потенциально_достижимая_клетка not in множество_достижимых_клеток:
                    ...
                    if потенциально_достижимая_клетка not in волны[номер_волны]:
                        ...
                        if check_if_there_are_forgotten_cells(множество_достижимых_клеток):
                            множество_достижимых_клеток.remove(волны[номер_волны].pop())
                            continue
                        ...
                        board_traversal(волны, номер_волны, множество_достижимых_клеток)

    if len(волны[номер_волны]) == 0:
        go_back(волны, номер_волны, множество_достижимых_клеток)
