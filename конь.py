            ...
            if 1 <= потенциально_достижимая_клетка[0] <= 8 and 1 <= потенциально_достижимая_клетка[1] <= 8:
                if потенциально_достижимая_клетка not in множество_достижимых_клеток:
                    ...
                    if потенциально_достижимая_клетка not in волны[номер_волны]:
                        ...
                        gui.draw_arrow(x, y, *потенциально_достижимая_клетка, цвет_исходной_точки)
                        time.sleep(0.5)

    множество_достижимых_клеток |= волны[номер_волны]

    цвет_исходной_точки = цвет_текущей_волны
...
while номер_волны > 1:
    ...
    gui.draw_arrow(x, y, *потенциально_достижимая_клетка, цвет_текущей_волны, 5)
    time.sleep(0.5)
    x, y = потенциально_достижимая_клетка
#заканчиваем первой
gui.draw_arrow(x, y, x0, y0, цвет_текущей_волны, 5)
time.sleep(0.5)
